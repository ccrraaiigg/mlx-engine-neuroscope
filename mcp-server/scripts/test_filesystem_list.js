...
async function main() {
  const proc = spawn('npx', ['-y', '@modelcontextprotocol/server-filesystem', process.cwd()], { stdio: ['pipe', 'pipe', 'inherit'] });
  ...
  proc.stdin.write(encode({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { clientInfo: { name: 'test', version: '0.0.0' }, capabilities: {} } }));
  proc.stdin.write(encode({ jsonrpc: '2.0', id: 2, method: 'tools/list' }));
  setTimeout(() => proc.stdin.end(), 1200);
}
